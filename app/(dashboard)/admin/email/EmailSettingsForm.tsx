...
export default function EmailSettingsForm({ initial }: Props) {
  ...
  async function handleSave(e: React.FormEvent) {
    ...
    const res = await fetch('/api/admin/email-settings', {
      ...
      body: JSON.stringify({ enabled, host, port, secure, username, password, fromAddress, fromName }),
    })
  ...
  return (
    <div className="space-y-8 max-w-2xl">
      ...
      <div className="bg-blue-50 border border-blue-200 rounded-md p-4 text-sm text-blue-800">
        <strong>Standard SMTP</strong> is supported. For <strong>Gmail</strong> you must enable an App Password
        (Google Account &rarr; Security &rarr; App Passwords) and use port 587 with STARTTLS (secure: off) or
        port 465 with SSL (secure: on). For <strong>Microsoft 365</strong> you must enable SMTP AUTH for the
        mailbox and use smtp.office365.com on port 587 (secure: off, STARTTLS negotiated automatically).
      </div>
