...
export default function ResourcesView({
  ...
  async function handleAddLink(e: React.FormEvent) {
    ...
    try {
      ...
      const data = await res.json()
      if (!res.ok) {
        setError(data.error ?? 'Failed to add web link')
        return
      }
  ...
  async function handleDelete(resourceId: string) {
    ...
    try {
      const res = await fetch(`/api/documents/${resourceId}`, { method: 'DELETE' })
  ...
  const tableHeaders = canDelete
    ? ['Name', 'Type', 'Category', 'Uploaded By', 'Date', '']
    : ['Name', 'Type', 'Category', 'Uploaded By', 'Date']
      ...
      {canUpload && (
        <>
          <form
            ...
            className="bg-white rounded-lg shadow px-6 py-5 mb-4 flex flex-wrap items-end gap-4"
          >
            <div>
          ...
          >
            ...
            <button
              ...
              className="rounded-md bg-primary text-white px-4 py-2 text-sm font-medium hover:opacity-90 transition-opacity disabled:opacity-50"
            >
              {addingLink ? 'Adding...' : 'Add Web Link'}
            </button>
          </form>
        </>
      )}
            ...
            <tbody className="divide-y divide-gray-200">
              {resources.map((r) => (
                <tr key={r.id}>
                  ...
                  <td className="px-6 py-4 text-sm text-gray-600">{r.uploaderName}</td>
