...
interface RouteContext {
  params: Promise<{ userId: string; taskId: string; attachmentId: string }>
}
...
export async function DELETE(_req: NextRequest, { params }: RouteContext) {
  ...
  const { userId, taskId, attachmentId } = await params
  ...
  const attachmentIdError = validateCuid(attachmentId, 'attachmentId')
  ...
  if (
    attachment.userTask.userId !== userId ||
    attachment.userTask.taskId !== taskId
  ) {
    return NextResponse.json({ error: 'Attachment not found' }, { status: 404 })
  }
